...
async function fill_library() {
  try {
    const API_URL =
      "https://guba-e-ethiopian-legal-advisor.onrender.com/get_processed_files";
    ...
    for (const fileName of files) {
      ...
      new_li.innerHTML = `
                    <li><span class="status-dot"></span> ${fileName}</li>
                `;
      libraryList.appendChild(new_li);
    }
  } catch (error) {
...
async function fetchGubaeResponse(userPrompt) {
  const API_URL = "https://guba-e-ethiopian-legal-advisor.onrender.com/chat";
...
form.addEventListener("submit", async (e) => {
  ...
  if (result.error) {
    ...
    const textAnswer =
      result.answer || "I'm sorry, I couldn't find a specific answer.";

    // 2. Format citations from list to string
    let formattedCitations = "";
    if (
      result.citation &&
      Array.isArray(result.citation) &&
